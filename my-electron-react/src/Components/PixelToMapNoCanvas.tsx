...
// === 2) Orientation mapping: fix 90/270 swaps (use correct W/H) ===
        function uvDisplayToSensor(uDisp: number, vDisp: number) {
            ...
            // natural sensor size is imgW x imgH
            switch (ori) {
                case "rotate 90 cw":
                case "right-top":
                case "6":
                    // display (uDisp,vDisp) -> sensor (u,v)
                    // 90° CW: (x',y') = (H - 1 - v, u)  ⇒ inverse: u = vDisp ; v = imgH - uDisp
                    return { u: vDisp, v: imgH - uDisp };
                case "rotate 270 cw":
                case "left-bottom":
                case "8":
                    // 270° CW: (x',y') = (v, W - 1 - u) ⇒ inverse: u = imgW - vDisp ; v = uDisp
